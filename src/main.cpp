...
int main(int argc, char **argv) {
  ...
  if (!horizon::theme_manager()->load_app_color_scheme("pluma-writer")) {
    std::cerr << "Warning: failed to load Pluma app color scheme; using system theme fallback\n";
  }
  if (!horizon::theme_manager()->activate_app_color_scheme("pluma-writer")) {
    std::cerr << "Warning: failed to activate Pluma app color scheme; using system theme fallback\n";
  }

  char *home = std::getenv("HOME");
  std::string config_path = home ? std::string(home) + "/.config/horizon/pluma-writer.json"
                                 : "pluma-writer.json";
  ...
  app.set_preferences_content(
      [config_path]() {
        ...
        content->add_section(horizon::i18n().tr("pluma-writer.preferences.general"),
                              "preferences-system",
